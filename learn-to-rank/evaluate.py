...
def err(predictions, labels, p=0):
    # from evaluate.py
    if p > 0:
        p = min(predictions.shape[0], p)
    else:
    ...
    def relevance(score):
        return (2**score - 1)/(2**max_score)

    p, err = 1, 0

    for i in range(p):
        rel = relevance(predictions[i])
        err += p * rel / (i+1)
        p *= (1-rel)
...
def evaluate(data_split, all_scores, print_results=False):
    ...
    for qid in np.arange(data_split.num_queries()):
        if included(qid, data_split):
            add_to_results(results, evaluate_query(
                data_split, qid, all_scores))

...
class CrossLingualPipeline:
    ...
    def get_dutch_sentence_embedding(self, sentence):
        ...
        sent_tokens = [word.lower() for word in sentence.split()]
        emb_sent = []
        used_tokens = []
        ...
        with torch.no_grad():
            if override_embeddings is None:
                ...
                for word in sentence.split():
                    try:
                        sentence_list.append(self.nl2en(word)[0])
            ...
            losses = self.ce_loss(prediction_scores.view(-1, model.config.vocab_size), all_word_labels.view(-1))
            ...
            predicted_distributions = torch.softmax(prediction_scores, dim=2)
            vals, args = torch.max(predicted_distributions, dim=2)
            ...
            return losses.cpu().numpy().tolist(), conf, predicted_tokens, actual_tokens, perplexity.cpu().numpy().tolist()
    ...
    def compare_embeddings(self):
        ...
        with open('data/filtered_en_nl_dict.txt', 'r', encoding="utf-8") as f:
            english_words_to_embed = [words.split('\t')[1] for words in f.read().splitlines()]

        with open('data/filtered_en_nl_dict.txt', 'r', encoding="utf-8") as f:
            dutch_words_to_embed = [words.split('\t')[0] for words in f.read().splitlines()]

        for word in dutch_words_to_embed:
            poly_word = Word(word, language="nl")
            dutch_word_embeddings[word] = poly_word.vector

        word_indices = torch.tensor(self.bert_tokenizer.encode(english_words_to_embed, add_special_tokens=True)[1:-1]).unsqueeze(0)
        all_word_embeddings = self.bert_model.bert.embeddings.word_embeddings
        english_embeddings = all_word_embeddings(word_indices)
        
        for index, english_word in enumerate(english_words_to_embed):
            english_word_embeddings[english_word] = english_embeddings.squeeze()[index]

        for dutch_word in dutch_word_embeddings:
            dutch_transformed_word = dutch_word_embeddings[dutch_word] @ self.transform.transform.detach().numpy()
            best_word = (None, 0)
            for english_word in english_word_embeddings:
                ...
                if similarity > best_word[1]:
                    best_word = (english_word, similarity)
            print('{} -> {}'.format(dutch_word, best_word[0]))
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--sentence_length_cutoff", help="sentence longer than this value get discarded", type=int, default=65)
    ...
    for sentence in tqdm(sentences):
        ...
        if not args.native:
            ...
            # native perplexity
            losses, conf, predicted_tokens, actual_tokens, perplexity = cross_lin_pipe.get_sentence_perplexity(sentence, cross_lin_pipe.dutch_bert_tokenizer, cross_lin_pipe.dutch_bert_model)
